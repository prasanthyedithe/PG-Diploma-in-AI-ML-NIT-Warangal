...
def print_all_feeds(entry):
    log.info("In print_all_feeds(entry)")
    try:
        for item in entry:
        ...
        raise Exception("Sorry, Error while Printing values from Entry", e)


def print_range_feeds(entry, limit_value):
    log.info("In print_range_feeds(entry, limit_value)")
    try:
        ...
        raise Exception("Sorry, Error while Printing values from Entry in Range", e)


def construct_json_all_feeds(entry, my_json_list):
    log.info("In construct_json_all_feeds(entry, my_json_list) ")
    try:
        for item in entry:
        ...
        raise Exception("Sorry, Error while construct_json_all_feeds", e)


def construct_json_range_feeds(entry, limit_value, my_json_list):
    log.info("In construct_json_range_feeds(entry, limit_value, my_json_list)")
    try:
        for item in range(0, limit_value):
        ...
        raise Exception("Sorry, Error while construct_json_range_feeds", e)
